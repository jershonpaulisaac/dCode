...
import { FileText, Layers, Database, GitBranch, Boxes, Terminal, ArrowLeft, HardDrive, ShieldAlert, ShieldCheck, TriangleAlert as AlertTriangle, Info, Route, Gauge, CircleCheck as CheckCircle2, Circle as XCircle, FileCode, Sparkles, Network, FolderTree } from 'lucide-react';
import type {
  AnalysisData,
  SecurityFinding,
  SecuritySeverity,
...
function AdvancedMetricsSection({ metrics }: { metrics: AdvancedMetric[] }) {
  if (!metrics || metrics.length === 0) return null;
  return (
    <SectionWrapper delay={0.32} className="p-7">
      <SectionHeader title="CodeLens AI Intelligence Signals" icon={Sparkles} />
        ...
        {metrics.map((metric) => {
          ...
          return (
            <div key={metric.label} className="rounded-xl border border-slate-800 bg-slate-900/50 p-5">
              ...
              {metric.unit !== '%' && <p className={`mt-3 text-[10px] uppercase tracking-wider ${color}`}>{metric.unit}</p>}
            </div>
          );
        })}
      </div>
...
export function Dashboard({ data, onReset }: DashboardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="mb-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg border border-slate-800 bg-slate-900/50 px-3 py-2 text-xs text-slate-400 transition-colors hover:border-slate-700 hover:text-slate-200"
          >
            <FolderTree className="h-3.5 w-3.5" />
            Project Explorer
          </button>
          <button
            onClick={() => setShowGraph((visible) => !visible)}
            className="inline-flex items-center gap-2 rounded-lg border border-cyan-500/20 bg-cyan-500/5 px-3 py-2 text-xs text-cyan-300 transition-colors hover:bg-cyan-500/10"
          >
      ...
      {showExplorer && <div className="mb-6"><ProjectExplorer fileTree={data.fileTree} /></div>}
      {showGraph && <div className="mb-6"><ArchitectureGraph nodes={data.architecture_nodes} /></div>}
        ...
        <div className="mb-4 flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              CodeLens AI analysis powered by IBM Bob Enterprise Agent
            </p>
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid gap-6">
        ...
        {data.advancedMetrics && <AdvancedMetricsSection metrics={data.advancedMetrics} />}
      ...
      <div className="mt-12 border-t border-slate-800/40 pt-6">
        <div className="flex items-center justify-between text-xs text-slate-600">
          <span>CodeLens AI — Enterprise AI Code Intelligence</span>
          <span className="inline-flex items-center gap-1.5">
